...
void ds3231_WriteTime(uint8_t hours, uint8_t minutes, uint8_t seconds, uint8_t date, uint8_t month, uint8_t year)
{
	...
	time_data[0] = DEC2BCD(seconds);
	time_data[1] = DEC2BCD(minutes);
	time_data[2] = DEC2BCD(hours);
	time_data[3] = DEC2BCD(1); // Day of week (fixed to 1)
	time_data[4] = DEC2BCD(date);
	time_data[5] = DEC2BCD(month);
	time_data[6] = DEC2BCD(year);
